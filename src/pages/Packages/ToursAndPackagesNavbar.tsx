...
import { User, Plane, Hotel, FileText, Shield, Calendar, Car } from 'lucide-react';

interface ToursAndPackagesNavbarProps {
  activeService?: 'flights' | 'hotels' | 'visa' | 'insurance' | 'tours' | 'cabs';
}

export default function ToursAndPackagesNavbar({
  activeService = 'tours',
}: ToursAndPackagesNavbarProps) {
  ...
  const navItems = [
    { id: 'flights', name: 'Flights', path: '/dashboard', icon: Plane },
    { id: 'hotels', name: 'Hotels', path: '/dashboard', icon: Hotel },
    { id: 'tours', name: 'Tours & Packages', path: '/dashboard', icon: Calendar },
    { id: 'cabs', name: 'Cabs', path: '/dashboard', icon: Car },
    { id: 'insurance', name: 'Insurance', path: '/dashboard', icon: Shield },
    { id: 'visa', name: 'Visas', path: '/dashboard', icon: FileText },
  ];

  return (
    <nav className="w-full bg-white p-3 z-50 flex justify-center border-b border-gray-100 font-sans">
      <div
      ...
      >
        {/* --- Logo Area --- */}
        <Link to="/dashboard" className="flex-shrink-0 flex items-center gap-2 group mr-auto">
          <img
            ...
            onError={(e) => {
              // Fallback to secondary logo path if standard image fails
                ...
                <Link
                  ...
                  className={`flex items-center gap-1.5 lg:gap-2 px-3 lg:px-5 py-2 rounded-full transition-all duration-300 font-semibold group whitespace-nowrap ${
                    isActive
                      ...
                      : 'text-gray-600 hover:text-[#1A1F4D] hover:bg-[#1A1F4D]/10'
                  }`}
                  style={{
                    backgroundColor: isActive ? '#1A1F4D' : 'transparent',
                ...
                >
                  <Icon
                    ...
                    className={`transition-colors duration-300 ${
                      isActive ? 'text-white' : 'text-gray-500 group-hover:text-[#1A1F4D]'
                    }`}
                  />
        ...
        <div className="relative flex items-center justify-end flex-shrink-0" ref={dropdownRef}>
          <div
            className="bg-[#1A1F4D] p-2 rounded-full cursor-pointer hover:bg-[#121638] transition-colors shadow-sm"
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center justify-center flex-shrink-0 px-2 h-full rounded-2xl transition-all duration-300 ${
                  isActive ? 'text-white scale-105' : 'text-gray-400 hover:text-[#1A1F4D]'
                }`}
                style={{
                  backgroundColor: isActive ? '#1A1F4D' : 'transparent',
                  minWidth: '52px',
