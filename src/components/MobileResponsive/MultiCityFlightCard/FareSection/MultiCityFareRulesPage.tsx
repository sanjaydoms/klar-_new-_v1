...
const MultiCityFareRulesPage: React.FC = () => {
  ...
  const getRulesForRoute = () => {
    if (!fareRuleData?.fareRule || !routeKey) {
      return null;
    }
    return fareRuleData.fareRule[routeKey];
  };

  const rules = getRulesForRoute();

  const getCancellationPolicy = () => {
    if (!rules?.tfr?.CANCELLATION) {
      return <p className="text-gray-500 text-sm">No data available</p>;
    }
    return rules.tfr.CANCELLATION.map((item, index) => (
      <div
        key={index}
        className="mb-3 pb-3 border-b border-gray-100 last:border-0 last:mb-0 last:pb-0 last:border-0"
      >
  ...
  const getDateChangePolicy = () => {
    if (!rules?.tfr?.DATECHANGE) {
      return <p className="text-gray-500 text-sm">No data available</p>;
    }
    return rules.tfr.DATECHANGE.map((item, index) => (
      <div
        key={index}
        className="mb-3 pb-3 border-b border-gray-100 last:border-0 last:mb-0 last:pb-0 last:border-0"
      >
  ...
  const getNoShowPolicy = () => {
    if (!rules?.tfr?.NO_SHOW) {
      return <p className="text-gray-500 text-sm">No data available</p>;
    }
    return rules.tfr.NO_SHOW.map((item, index) => (
      <div
        key={index}
        className="mb-3 pb-3 border-b border-gray-100 last:border-0 last:mb-0 last:pb-0 last:border-0"
      >
        <div className="text-gray-700 text-sm">{item.policyInfo}</div>
        <div className="text-xs text-gray-500 mt-1">
          {item.st}h - {item.et}h
        </div>
      </div>
  ...
  const getSeatPolicy = () => {
    if (!rules?.tfr?.SEAT_CHARGEABLE) {
      return <p className="text-gray-500 text-sm">No data available</p>;
    }
    return rules.tfr.SEAT_CHARGEABLE.map((item, index) => (
      <div
        key={index}
        className="mb-3 pb-3 border-b border-gray-100 last:border-0 last:mb-0 last:pb-0 last:border-0"
      >
  ...
  const getTotalFareDisplay = () => {
    ...
    const total = selectedFare.priceSummary?.AdultFare?.total || 0;
    return `₹${total.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
  };

  const getFareTypeDisplay = () => {
    if (!selectedFare) return '';
    return selectedFare.FareIdentifierType || 'Standard';
  };
  ...
  return (
    <div className="block md:hidden lg:hidden min-h-screen bg-gray-50 p-3 sm:p-4 pb-24">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-1">Fare Rules</h1>
        <p className="text-sm text-gray-500 mb-1">Route {routeIndex + 1}</p>
        <div className="flex items-center gap-3 mb-4">
          <span className="text-sm font-semibold text-[#EF4444]">{getTotalFareDisplay()}</span>
        ...
        <div className="bg-white rounded-xl border border-input p-4 sm:p-5 mt-4 sm:mt-6">
          ...
          >
            {isChecked ? '✓ Complete Route' : 'Please accept to continue'}
          </button>
        </div>
      </div>
    </div>
