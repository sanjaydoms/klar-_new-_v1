...
// import { getWalletInfo, deductWalletBalance } from '@/api/wallet.api';
...
export default function CabReviewPage() {
  ...
  // NOTE: AuthContext does not expose `token` — use `isAuthenticated` (redux
  // user + token) to decide whether auth-only calls (wallet/markup) may run.
  ...
  useEffect(() => {
    ...
    // No auth gate here: guests can book cabs — the backend treats tokenless
    // bookings as GUEST and collects payment via Razorpay. Wallet/markup are
    // auth-only endpoints, so skip them for guests (they 401 otherwise; guests
    // simply get no markup applied).
    if (isAuthenticated) {
      fetchWalletInfo();
      fetchMarkups();
    }
  ...
  const handleBooking = async () => {
    ...
    if (!travellerForm.firstName.trim()) errors.firstName = 'First name is required';
    if (!travellerForm.lastName.trim()) errors.lastName = 'Last name is required';
    ...
    try {
      ...
      // The client-side auth user object doesn't reliably carry an id (OTP login
      // never populates one), so resolve the real userId from the backend the
      // same way the flights booking flow does — never fall back to a fixed
      // 'guest' string for a logged-in user.
      let resolvedUserId = '';
      ...
      if (isAuthenticated) {
        try {
          ...
          resolvedUserEmail = profile?.email || '';
          resolvedMobile = profile?.mobile || '';
        } catch (err) {
          logger.error('Failed to fetch user profile for booking:', err);
        }
      }
      ...
      if (selectedPayment === 'razorpay') {
        const paymentData = {
          ...
          userEmail: resolvedUserEmail || travellerForm.email || 'guest@klar.com',
          mobile: resolvedMobile || travellerForm.phone || '9999999999',
          clientType: 'B2C',
        };
      ...
      const bookingPayload = {
        ...
        addons: [],
        routeDetail: {
          // Cross-border/international routes must be flagged as non-domestic —
          // sending isDomestic:true for e.g. a Singapore pickup gets the booking
          // rejected by the supplier (SUPPLIER_ERROR). Derive it from the actual
          // origin/destination country captured at search time.
          isDomestic:
            (searchParams.origin?.address?.country || 'India').toLowerCase() === 'india' &&
            (searchParams.destination?.address?.country || 'India').toLowerCase() === 'india',
          origin: {
            displayAddress: searchParams?.from || 'Origin',
            lat: searchParams.origin?.lat,
            long: searchParams.origin?.long,
            address: searchParams.origin?.address,
          },
          destination: {
            displayAddress: searchParams?.to || 'Destination',
            lat: searchParams.destination?.lat,
            long: searchParams.destination?.long,
            address: searchParams.destination?.address,
          },
        },
        quotationInfo: {
        ...
        agentEmail: resolvedUserEmail || travellerForm.email || 'agent@klar.com',
        agentPhone: resolvedMobile || travellerForm.phone || '+919876543210',
            ...
            <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-8 py-12 text-center text-white relative">
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div className="w-20 h-20 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center mb-6 border border-white/30 animate-bounce">
                  {bookingSuccess?.status === 'PAYMENT_PENDING' || bookingSuccess?.status === 'SUPPLIER_PENDING' || bookingSuccess?.status === 'PENDING' ? (
            ...
            <div className="p-8 md:p-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="space-y-10">
                  ...
                  {bookingSuccess.policies?.amendmentPolicy && (
                    <section className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                  ...
                  <div className="bg-orange-50 rounded-[2rem] p-8 border border-orange-100">
                    ...
                    <div className="space-y-4 mb-6">
                      ...
                      <div className="flex justify-between text-[10px]">
                        ...
                        <span className="font-bold text-orange-600 uppercase">Wallet</span>
                      </div>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <div className="mt-8 relative">
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                  <div className="flex-1">
                    ...
                    <h3 className="font-medium text-base sm:text-lg md:text-[20px] text-gray-900 leading-[140%] tracking-normal">
                      {searchParams?.from || ''}
                    </h3>
                  </div>
                  ...
                  <div className="hidden md:flex items-center gap-1 shrink-0">
                    <div className="w-2.5 h-2.5 rounded-full border-2 border-orange-500 bg-white shrink-0" />
                    <div className="w-16 border-t-2 border-dashed border-gray-300" />
                    <div className="w-2.5 h-2.5 rounded-full border-2 border-orange-500 bg-white shrink-0" />
                  </div>
                  
                  {/* Mobile connector */}
                  <div className="md:hidden flex flex-col items-center justify-center shrink-0 w-8">
                    <div className="w-2.5 h-2.5 rounded-full border-2 border-orange-500 bg-white shrink-0" />
                ...
                <div className="mt-6 flex flex-wrap items-center gap-4">
                  <div className="flex items-center gap-2 text-xs font-bold text-gray-600 bg-gray-50 p-2 rounded-lg inline-flex">
                    <span className="flex items-center gap-2 text-sm font-medium text-gray-700">
                      ...
                      {new Date(searchParams.pickupDate).toLocaleString('en-IN', {
                        day: '2-digit',
                        month: 'short',
                        year: 'numeric',
                      })}{' '}
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-900 break-words whitespace-normal">
                      {selectedCab?.vehicleName || ''}
                    </h2>
                    {selectedCab?.rating && (
                      <div className="flex items-center gap-1.5 bg-green-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm self-start sm:self-auto">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 w-full">
              {/* Steps Header with Connecting Lines - LONGER LINES */}
              <div className="hidden sm:flex items-center justify-between mb-6 w-full px-4">
                <div className="flex items-center gap-6 flex-1">
                  <div className="px-4 py-1.5 bg-gray-200 text-gray-700 text-xs font-bold rounded-md whitespace-nowrap">
                    Step 1
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                <div className="text-left">
                  <h3 className="font-bold text-gray-900 text-sm mb-1">Booking Confirmed</h3>
                  <p className="text-xs text-gray-500 leading-relaxed">
                    You'll receive your booking voucher right after the payment.
                  </p>
                </div>
                <div className="text-left">
                  <h3 className="font-bold text-gray-900 text-sm mb-1">Cab & Driver Details</h3>
                  <p className="text-xs text-gray-500 leading-relaxed">
                    We'll share your cab and driver details 24 hours before your trip.
                  </p>
                </div>
                <div className="text-left">
                  <h3 className="font-bold text-gray-900 text-sm mb-1">On-Time Pickup</h3>
                  <p className="text-xs text-gray-500 leading-relaxed">
                    Your cab will arrive at the pickup point at the scheduled time. Enjoy your trip!
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-8 space-y-6">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-black text-gray-900">
                      ₹
                      {((baseFare || 0) + cabMarkupAmount).toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </span>
