...
import { agreedValue, formatBaggage } from '@/features/flights/components/FlightCardFooter';
...
const MultiCityInternationalFlightCard: React.FC<MultiCityInternationalFlightCardProps> = ({
  ...
  // One combo fare prices every leg, but the legs can state different
  // allowances; show a value only when they agree.
  const refundable = agreedValue(itinerary.legs.map((l) => l.refundable));
  const baggage = formatBaggage(
    agreedValue(itinerary.legs.map((l) => l.checkInBaggage)),
    agreedValue(itinerary.legs.map((l) => l.cabinBaggage)),
  );
    ...
    return (
      <div className="flex-1">
        ...
        <div className="flex items-center space-x-1 mb-1 sm:mb-1.5">
          ...
          {formatAircraft(leg.aircraftTypes) && (
            <>
        ...
        <div className="mb-0.5">
          ...
          <div className="text-[8px] sm:text-[9px] text-gray-500 font-medium pl-5">
            {leg.from.airportCode || 'N/A'}
            {formatTerminal(leg.from.terminal) && (
              <span className="text-gray-400"> {formatTerminal(leg.from.terminal)}</span>
            )}{' '}
            • {leg.from.city || 'N/A'}
          </div>
        ...
        <div className="relative my-0.5 sm:my-1">
          ...
          <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white px-0.5 sm:px-1">
            <div className="text-center">
              ...
              <div className="text-[6px] sm:text-[7px] text-[#EF4444]">{getStopDisplay(leg)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="text-right mb-0.5 sm:mb-1">
          ...
          <div className="text-[8px] sm:text-[9px] text-gray-500 font-medium pr-5">
            {leg.to.airportCode || 'N/A'}
            {formatTerminal(leg.to.terminal) && (
              <span className="text-gray-400"> {formatTerminal(leg.to.terminal)}</span>
            )}{' '}
            • {leg.to.city || 'N/A'}
          </div>
  ...
  return (
    <div
      className={`bg-white rounded-xl border ${
        isSelected || isComplete ? 'border-green-500 border-2' : 'border-[#0A2662]'
        ...
        {!isExpanded && itinerary.legs.length > 2 && (
          <div className="text-center mt-1">
            <span className="text-[8px] sm:text-[9px] text-blue-600 font-medium">
              + {itinerary.legs.length - 2} more leg{itinerary.legs.length - 2 > 1 ? 's' : ''} •
              Click to expand
            </span>
          </div>
        )}
