 ...
 *
 * OBSERVE ONLY — no routing, no circuit breaker.
 *
 * TripJack is KLAR's only flight supplier. Routing has exactly one candidate,
 * so consulting it could only ever return the same answer, and a breaker could
 * only ever refuse to sell a flight nobody else can sell. Both would be
 * ceremony that costs availability.
 *
 * That changes the day a second flight aggregator exists — and it is the case
 * routing was built for, because two aggregators genuinely CAN both answer a
 * flight search. Until then this service reports and nothing more.
 *
...
export const flush = async (): Promise<void> => {
  ...
  try {
    await axios.post(
      `${envConfig.INTEGRATION_SERVICE_URL}/internal/telemetry`,
      { reports: batch, reportedBy: `flight@${os.hostname()}:${process.pid}` },
