 ...
 *
 * NO CIRCUIT BREAKER HERE, and that is deliberate. This service prices and
 * books against a SPECIFIC rate at a SPECIFIC supplier: there is no second
 * provider who could serve the same request, so refusing to call the one that
 * owns the booking would not protect anything — it would abandon a customer
 * mid-purchase. The breaker belongs on search, where a fan-out makes skipping
 * a supplier merely slower rather than fatal.
 *
 * So this service OBSERVES and does not decide. That is the whole difference
 * between the two integrations, and it is a property of the domain rather than
 * an omission.
 *
...
export const flush = async (): Promise<void> => {
  ...
  try {
    await axios.post(
      ...
      { reports: batch, reportedBy: `hotel-book@${os.hostname()}:${process.pid}` },
